"""
Run M3 Experiment: EMSC + Augmentation + CNN for Mortality Classification
Complete pipeline execution for mortality prediction using advanced preprocessing and CNN
"""
...
from M3_preprocessing import MortalityEMSCProcessor
from M3_model import MortalityCNN
...
def load_mortality_data():
    ...
    valid_mortality = mortality_df[mortality_column].isin(list(mortality_mapping.keys()))
    ...
    print(f"  - Mortality distribution: {dict(pd.Series(mortality_df[mortality_column]).value_counts())}")
...
def main():
    """Run complete M3 experiment"""
    print("="*80)
    print("M3 EXPERIMENT: EMSC + AUGMENTATION + CNN FOR MORTALITY")
    print("="*80)
    ...
    try:
        ...
        print(f"\nInitializing M3 EMSC preprocessor...")
        ...
        X_train_processed, y_train_processed = preprocessor.preprocess_mortality_data(
        ...
        print(f"\nInitializing M3 CNN model...")
        model = MortalityCNN(random_state=42)
        
        # Train the CNN
        training_results = model.train_mortality_cnn(X_train_processed, y_train_processed)
        ...
        # Analyze CNN features
        feature_analysis = model.analyze_cnn_features(
            X_test_processed, preprocessor.preprocessing_stats
        )
        ...
        # Print final summary
        print("\n" + "="*80)
        print("M3 EXPERIMENT COMPLETED SUCCESSFULLY")
        print("="*80)
        
        best_model = evaluation_results['best_model']['name'].upper()
        ...
        print(f"CNN Input Shape: {X_train_processed.shape}")
        print(f"Data Augmentation: {preprocessor.preprocessing_stats['n_samples_original']} → {preprocessor.preprocessing_stats['n_samples_augmented']} samples")
        
        # Model comparison
        print(f"\nModel Performance Comparison:")
        cnn_acc = evaluation_results['cnn']['accuracy']
        cnn_auc = evaluation_results['cnn']['auc']
        backup_acc = evaluation_results['backup']['accuracy']
        backup_auc = evaluation_results['backup']['auc']
        
        print(f"  CNN: {cnn_acc:.4f} (AUC: {cnn_auc:.4f})")
        print(f"  Backup RF: {backup_acc:.4f} (AUC: {backup_auc:.4f})")
        
        # CNN specific metrics
        if model.training_history:
            final_train_acc = model.training_history['accuracy'][-1]
            final_val_acc = model.training_history['val_accuracy'][-1]
            epochs_trained = len(model.training_history['loss'])
            
            print(f"\nCNN Training Details:")
            print(f"  Epochs trained: {epochs_trained}")
            print(f"  Final training accuracy: {final_train_acc:.4f}")
            print(f"  Final validation accuracy: {final_val_acc:.4f}")
            print(f"  Model parameters: {training_results['model_architecture']['total_params']:,}")
        
        return True
        
    except Exception as e:
        print(f"\n❌ M3 experiment failed: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n💥 M3 experiment failed. Check error messages above.")
        sys.exit(1) 